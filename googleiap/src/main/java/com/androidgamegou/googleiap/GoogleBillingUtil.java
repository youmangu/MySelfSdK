...
public class GoogleBillingUtil {
    ...
    public String[] subsSKUS = new String[]{};//订阅ID,必填
    ...
    private class MyPurchasesUpdatedListener implements PurchasesUpdatedListener
    {
        ...
        public void onPurchasesUpdated(BillingResult billingResult, List<Purchase> list) {
            ...
            {
                //Log.d("Unity", "================ mOnPurchaseFinishedListener is null");
                if(IS_DEBUG)
                {
            ...
            //Log.d("Unity", "================ mOnPurchaseFinished:  " + responseCode + " " + isAutoConsumeAsync + " " + list.toString());
    ...
    {
        Purchase.PurchasesResult reward = mBillingClient.queryPurchases(BillingClient.SkuType.INAPP);
        if (reward.getResponseCode() == 0){
            List<Purchase> pur = reward.getPurchasesList();
            for (int i= 0; i<pur.size();i++){
                UnityPlayer.UnitySendMessage("IAPListener","GooglePayResultMessgae","Success;"+pur.get(i).getSku()
                        +";"+pur.get(i).getSignature()+";"+pur.get(i).getOriginalJson());
            }
    ...
    public void QuerySkuDetail(String BillingSku) {
        String[] skus = BillingSku.split(";");
        List<String> skusList = new ArrayList<>();
        for (int i = 0; i< skus.length; i++) {
            skusList.add(skus[i]);
        }
        SkuDetailsParams sk = new SkuDetailsParams();
        sk.newBuilder().setType(BillingClient.SkuType.INAPP).setSkusList(skusList).build();
        mBillingClient.querySkuDetailsAsync(sk, new SkuDetailsResponseListener() {
            @Override
            public void onSkuDetailsResponse(BillingResult billingResult, List<SkuDetails> skuDetailsList) {
                if(billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK && skuDetailsList != null){
                    ...
                    for (SkuDetails s:skuDetailsList){
                        sku_details += s.getSku()+",";
                        sku_details += s.getIntroductoryPrice()+";";
                        log("================================ googleBilling ");
                        log("getDescription: " + s.getDescription());
                        log("getIntroductoryPriceAmountMicros: " + s.getIntroductoryPriceAmountMicros());
                        log("getIntroductoryPrice: " + s.getIntroductoryPrice());
                        log("getIntroductoryPriceCycles: " + s.getIntroductoryPriceCycles());
                        log("getIntroductoryPricePeriod: " + s.getIntroductoryPricePeriod());
                        log("================================ googleBilling  end");
                        if(skuDetailsMap.get(s.getSku()) == null) {
                            skuDetailsMap.put(s.getSku(), s);
                        }
                    }
                    //log(" 返回price  "+sku_details);
    ...
    private class MyConsumeResponseListener implements ConsumeResponseListener
    {
        ...
        public void onConsumeResponse(BillingResult billingResult, String s) {
            if (billingResult.getResponseCode() == BillingClient.BillingResponseCode.OK) {
                UnityPlayer.UnitySendMessage("IAPListener", "message", "ConsumeSuccessed");
            }
        }
    }
    ...
    {
        return null;//queryPurchases(BillingClient.SkuType.SUBS);
    }
            ...
            if(purchasesResult!=null)
            {
                if(purchasesResult.getResponseCode()== BillingClient.BillingResponseCode.OK)
                {
                    ...
                    for (int i= 0; i<purchaseList.size();i++){
                        UnityPlayer.UnitySendMessage("IAPListener","QueryPurchasesedResult","Success;"+purchaseList.get(i).getSku()
                                +";"+purchaseList.get(i).getSignature()+";"+purchaseList.get(i).getOriginalJson()+";"+ purchaseList.get(i).getPurchaseToken());
                    }
                    if(isAutoConsumeAsync)
                    {
...
//        }
    ...
    /**
     * 清除所有监听器，防止内存泄漏isAutoConsumeAsync
     *
